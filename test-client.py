...
class Myo(myo.MyoRaw):
    ...
    def __init__(self, cls):
        ...
        self.emg_list = open('emg.txt', 'r').readlines()
        self.emg_head = 0
    ...
    def run(self, seconds):
        data = self.emg_list[self.emg_head]
        self.emg_head = (self.emg_head + 1) % len(self.emg_list)
        self.on_emg(map(lambda x: int(x), data.split(',')), 0)
        time.sleep(0.02)
        return data
